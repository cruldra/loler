...
from models import User, RunePage, TeamComposition, ChampionFavorite, HighlightVideo, ChampionTip, ItemBuild, Replay, EquipmentComparison
from routes import summoner, champion, item, team_composition, highlight, monster, replay, game_data, equipment_comparison
    ...
    if user:
        ...
        if db_user:
            ...
            replays = session.exec(
                select(Replay).where(Replay.user_id == db_user.id).order_by(Replay.created_at.desc())
            ).all()
